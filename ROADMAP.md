...
Next, validate a narrow TypeScript example adapter against the owning package's installed compiler, exact dependency versions, and configuration. Prefer existing documentation-test/build tools in other ecosystems, adding adapters according to usage. Incomplete snippets, ambiguous targets, and missing toolchains must remain unsupported or unavailable. Compiler execution belongs to explicit validation, not every hook. The connected assistant can help configure a repeatable check once; Mason retains its evidence. Automatic compiler selection and multi-language snippet execution are not implemented.

Evaluate detector precision and actionability with valid alternatives and ambiguous-scope controls before broadening default checks. The exploratory compiler and Turbo hash experiments establish useful candidates, not population accuracy or better agent outcomes. This foundation supports adoption and executable constraints; it does not replace the correction-capture and maintenance milestone above.
