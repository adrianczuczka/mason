...
function createCLI() {
  ...
  program2.name("foreman").description(
...
${totalFindings} findings from ${results.length} analyzers`)
  ...
  program2.command("init").description("Analyze codebase and generate CLAUDE.md").argument("[dir]", "Directory to analyze", ".").action(async (dir) => {
    const rootDir = path3.resolve(dir);
    const spinner = ora("Analyzing codebase...").start();
    const context = await buildContext(rootDir);
    const results = await runAll(context);
  ...
  program2.command("update").description("Re-analyze and update existing CLAUDE.md").argument("[dir]", "Directory to analyze", ".").action(async (_dir) => {
