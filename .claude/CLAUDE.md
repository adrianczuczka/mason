...
- Audit discovery includes nested README and instruction files with actual filename casing, bounded reads, and exclusions. Keep discovery and scoped claim dependencies shared with automation. Ambiguous path/command and unmentioned-module candidates are advisory with `resolution: recheck`: a rerun can establish that their condition is gone, without asserting semantic approval. Historical dependency and decision advisories still require separate assessment. Preserve original documents, scope, and skipped-input diagnostics through repair and the final commit.
