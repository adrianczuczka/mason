...
On session start, Mason recovers the current branch/worktree's evidence. Before and after potentially mutating tools, it checks for changed audit inputs and retains newly observed findings before another documentation edit can hide them. Shell and unknown tool calls are included because edits can happen outside a file-edit tool. Known read-only tools record lifecycle observations without rescanning the checkout or advancing the last audit verdict. Prompt submission and turn completion still run checks. A relevant unresolved issue can request **one continuation per session**; advisories and unavailable checks never create a repair loop. Fixes remain the assistant’s responsibility within the user’s task scope. Unrelated findings should be reported with a brief scope reason and a suggested follow-up; hooks do not authorize unrelated fixes, tickets, or advisory approval.
